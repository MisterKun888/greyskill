...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    ...
    let missions = db.findMany('missions')
    ...
    const enriched = missions.map(m => {
      const client = db.findById('users', m.clientId)
      const profil = m.seniorId ? db.findById('profils_seniors', m.seniorId) : null
      const seniorUser = profil ? db.findById('users', profil.userId) : null
      const candidatures = db.findMany('candidatures', { missionId: m.id })
      return {
        ...m,
  ...
  if (req.method === 'POST') {
    ...
    const mission = db.create('missions', {
    ...
    const profils = db.findMany('profils_seniors')
