...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    const messages = db.findMany<any>('messages', { missionId: missionId as string })
    const enriched = messages.map(m => {
      const sender = db.findById<any>('users', m.senderId)
  ...
  if (req.method === 'POST') {
    ...
    const message = db.create<any>('messages', {
    ...
    const sender = db.findById<any>('users', user.id)
