...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (action === 'create') {
    const mission = db.findById<any>('missions', missionId)
    ...
    const paiement = db.create<any>('paiements', {
    ...
    db.update<any>('missions', missionId, { statut: 'EN_COURS' })

    // Email simulé au client
    const client = db.findById<any>('users', user.id)
    sendEmailLog(
  ...
  if (action === 'capture') {
    ...
    const paiements = db.findMany<any>('paiements', { missionId })
    ...
    // Libérer le paiement
    db.update<any>('paiements', paiement.id, { statut: 'LIBERE', dateCapture: new Date().toISOString() })
    db.update<any>('missions', missionId, { statut: 'TERMINEE', dateFin: new Date().toISOString() })

    // Mise à jour certification senior
    const profil = db.findOne<any>('profils_seniors', { userId: mission.seniorId })
    if (profil) {
      ...
      db.update<any>('profils_seniors', profil.id, { nbMissions, certification })
    }

    // Emails simulés
    const client = db.findById<any>('users', mission.clientId)
  ...
  if (action === 'refund') {
    ...
    db.update<any>('paiements', paiement.id, { statut: 'REMBOURSE' })
    db.update<any>('missions', missionId, { statut: 'LITIGE' })
